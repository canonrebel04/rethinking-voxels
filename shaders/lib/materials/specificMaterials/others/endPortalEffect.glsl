...
#endif
float dismult = 0.5;
// ⚡ Bolt: Loop interchange optimization to move invariant calculations out of the inner loop
for (int i = 1; i <= 8; i++) {
	...
	// ⚡ Bolt: Replaced pow(..., 2.0) with pow2() to avoid double evaluation
	vec2 offset = vec2(0.0, 1.0/(3600.0/24.0)) * pow2(16.0 - float(i)) * 0.004;
	vec3 wpos_base = normalize((gbufferModelViewInverse * vec4(viewPos * (float(i) * dismult + 1.0), 1.0)).xyz);
