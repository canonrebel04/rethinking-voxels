...
for (int i = 1; i <= 8; i++) {
	float fi = float(i);
	float colormult = 0.9 / (30.0 + fi);
	float rotation = (fi - 0.1 * fi + 0.71 * fi - 11.0 * fi + 21.0) * 0.01 + fi * 0.01;
	float Cos = cos(radians(rotation));
	float Sin = sin(radians(rotation));
	float t = 16.0 - fi;
	vec2 offset = vec2(0.0, 1.0 / (3600.0 / 24.0)) * (t * t) * 0.004;

	vec3 wpos_i = normalize((gbufferModelViewInverse * vec4(viewPos * (fi * dismult + 1.0), 1.0)).xyz);
	vec2 wind_base = (frameTimeCounter + 984.0) * (fi + 8.0) * 0.125 * offset;
	vec2 wind = fract(wind_base);
	...
	for (int j = 0; j < repeat; j++) {
		...
		if (abs(NdotU) > 0.9) {
			...
			wpos.xz *= abs(playerPos.y) + fi * dismult + add;
		...
		if (mod(fi, 4.0) < 1.5) coord = coord.yx + vec2(-1.0, 1.0) * wind.y;
