...
for (int i = 1; i <= 8; i++) {
	...
	float diff = 16.0 - fi;
	vec2 offset = vec2(0.0, 1.0 / (3600.0 / 24.0)) * (diff * diff) * 0.004;
	vec2 wind = fract((frameTimeCounter + 984.0) * (fi + 8.0) * 0.125 * offset);

	for (int j = 0; j < repeat; j++) {
		float add = float(j + dither) * 0.0625 / float(repeat);
		vec3 wpos = normalize((gbufferModelViewInverse * vec4(viewPos * (fi * dismult + 1.0), 1.0)).xyz);
		if (abs(NdotU) > 0.9) {
			wpos.xz /= wpos.y;
