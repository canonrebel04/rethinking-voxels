		...
		if (length(normalDepthData.xyz) > 0.5) {
			...
			if (clamp(pos.xyz, -pointerGridSize * POINTER_VOLUME_RES / 2.0, pointerGridSize * POINTER_VOLUME_RES / 2.0) == pos.xyz) {
				...
				for (int lightNum = 0; lightNum < lightCount; lightNum++) {
					...
					brightness = max(ndotl, sssBrightness) * 0.0625 * lightBrightness * pow2(max(0.0, 1.0 - brightness / lightBrightness)); // ⚡ Bolt: replaced pow(x, 2) with pow2(x) for faster computation
					if (brightness > 0.01) {
