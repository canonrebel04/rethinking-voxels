...
bool getOcclusion(int lightPointer, vec3 pos0) {
	...
	float atten = max(0.0, 1.0 - length(dir) / lightBrightness);
	float brightness = 0.0625 * lightBrightness * (atten * atten); // ⚡ Bolt: Replace pow(x,2) with explicit multiplication
