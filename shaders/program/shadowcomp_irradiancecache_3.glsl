...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	brightness = 0.0625 * lightBrightness * pow2(max(0.0, 1.0 - brightness / lightBrightness)); // ⚡ Bolt: replaced pow(x, 2) with pow2(x) for faster computation
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float brightness = 0.0625 * lightCols[n].a * pow2(max(0.0, 1.0 - length(dir) / lightCols[n].a)); // ⚡ Bolt: replaced pow(x, 2) with pow2(x) for faster computation
			if (brightness > 0.01) {
