...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	float atten = max(0.0, 1.0 - brightness / lightBrightness);
	brightness = 0.0625 * lightBrightness * (atten * atten); // ⚡ Bolt: Replace pow(x,2) with explicit multiplication
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float atten = max(0.0, 1.0 - length(dir) / lightCols[n].a);
			float brightness = 0.0625 * lightCols[n].a * (atten * atten); // ⚡ Bolt: Replace pow(x,2) with explicit multiplication
			if (brightness > 0.01) {
