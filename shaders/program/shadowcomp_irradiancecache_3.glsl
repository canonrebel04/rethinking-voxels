...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	float t1 = max(0.0, 1.0 - brightness / lightBrightness);
	brightness = 0.0625 * lightBrightness * t1 * t1;
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float t2 = max(0.0, 1.0 - length(dir) / lightCols[n].a);
			float brightness = 0.0625 * lightCols[n].a * t2 * t2;
			if (brightness > 0.01) {
