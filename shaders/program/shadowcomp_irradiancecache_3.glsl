...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	float bDist = max(0.0, 1.0 - brightness / lightBrightness);
	// OPTIMIZATION: Replacing pow(x, 2) with x * x avoids expensive pow() calls
	brightness = 0.0625 * lightBrightness * (bDist * bDist);
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float bDist2 = max(0.0, 1.0 - length(dir) / lightCols[n].a);
			// OPTIMIZATION: Replacing pow(x, 2) with x * x avoids expensive pow() calls
			float brightness = 0.0625 * lightCols[n].a * (bDist2 * bDist2);
			if (brightness > 0.01) {
