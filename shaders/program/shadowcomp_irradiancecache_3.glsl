...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	// ⚡ Bolt: Use explicit multiplication instead of pow(x, 2) for performance and safety
	float temp1 = max(0.0, 1.0 - brightness / lightBrightness);
	brightness = 0.0625 * lightBrightness * (temp1 * temp1);
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			// ⚡ Bolt: Use explicit multiplication instead of pow(x, 2) for performance and safety
			float temp2 = max(0.0, 1.0 - length(dir) / lightCols[n].a);
			float brightness = 0.0625 * lightCols[n].a * (temp2 * temp2);
			if (brightness > 0.01) {
