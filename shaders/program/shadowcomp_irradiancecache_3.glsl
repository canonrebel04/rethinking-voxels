...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	float falloff = max(0.0, 1.0 - brightness / lightBrightness);
	brightness = 0.0625 * lightBrightness * (falloff * falloff);
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float falloff = max(0.0, 1.0 - length(dir) / lightCols[n].a);
			float brightness = 0.0625 * lightCols[n].a * (falloff * falloff);
			if (brightness > 0.01) {
