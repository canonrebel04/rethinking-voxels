...
vec4 getLightCol(int lightPointer, inout vec3 pos0) {
	...
	brightness = 0.0625 * lightBrightness * pow2(max(0, 1 - brightness / lightBrightness)); // ⚡ Bolt: Used pow2() for faster squaring
...
void main() {
	...
	if (doLighting) {
		for (int n = 0; n < lightCount; n++) {
			...
			float brightness = 0.0625 * lightCols[n].a * pow2(max(0, 1 - length(dir) / lightCols[n].a)); // ⚡ Bolt: Used pow2() for faster squaring
			if (brightness > 0.01) {
